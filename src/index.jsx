...
function App () {
    ...
    useEffect(() => {
        store.dispatch(
            photosByAlbum([state.app.selectectedAlbumId,
    ...
    return (
        <Provider store={store}>
            <div className="App">
                <Modal/>
                <h1 className="uppercase text-6xl text-blue-900 font-bold leading-none tracking-wide m-10">
                    Simple Gallery ({state.albums.length})
                </h1>
                <div className="grid rid-flow-col grid-cols-1 gap-5 ">
                    {state.albums.map((m, index) => {
                        return (
                            <div tabIndex={index} role="button" key={m.id} onClick={() => handleClick(m.id)}
                                className="group hover:bg-gray-200 p-4 cursor-pointer bg-white rounded 
